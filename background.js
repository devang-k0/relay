...
async function stopRecording(reason = "user") {
  ...
  try {
    ...
    await closeOffscreenDocument();
    state = { status: "idle", tabId: null, startedAt: null, lastError: "Save dialog failed to open." };
    await persistState();
    return { ok: false, error: "Save dialog failed to open." };
  }

  // The blob: URL only needs to stay alive long enough for the download
  // to start reading it. Closing the offscreen document now revokes it
  // automatically (blob URLs die with the document that created them).
  await closeOffscreenDocument();
